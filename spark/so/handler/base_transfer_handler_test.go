...
func TestCreateTransfer_CounterSwapV3_FailsWithNonPrimarySwapTransfer(t *testing.T) {
	config := sparktesting.TestConfig(t)
	ctx, _ := db.ConnectToTestPostgres(t)
	client, err := ent.GetDbFromContext(ctx)
	require.NoError(t, err)

	// The linked transfer has the right status, amount, network, and reversed parties,
	// but it is not a PRIMARY_SWAP_V3 transfer.
	alicePub := keys.GeneratePrivateKey().Public()
	bobPub := keys.GeneratePrivateKey().Public()

	nonSwapTransfer, err := client.Transfer.Create().
		SetSenderIdentityPubkey(alicePub).
		SetReceiverIdentityPubkey(bobPub).
		SetStatus(st.TransferStatusSenderKeyTweakPending).
		SetTotalValue(1000).
	...
	require.Error(t, err)
	require.ErrorContains(t, err, "has invalid type")
}
