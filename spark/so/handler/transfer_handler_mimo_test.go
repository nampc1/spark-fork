...
func TestCheckIfKeyTweakApplied(t *testing.T) {
	sparktesting.RequireGripMock(t)
	ctx, sessionCtx := db.ConnectToTestPostgres(t)
	...
	t.Run("returns true when combined key equals verifying pubkey", func(t *testing.T) {
		// Create a signing keyshare with a known key.
		sparkPrivKey := keys.MustGeneratePrivateKeyFromRand(rng)
		userPrivKey := keys.MustGeneratePrivateKeyFromRand(rng)

		// The "correct" verifying pubkey is spark + user combined.
		combinedPubKey := sparkPrivKey.Public().Add(userPrivKey.Public())

		ownerPubKey := keys.MustGeneratePrivateKeyFromRand(rng).Public()
		tree := createTestTreeForClaim(t, ctx, ownerPubKey, sessionCtx.Client)

		keyshare, err := sessionCtx.Client.SigningKeyshare.Create().
			SetStatus(st.KeyshareStatusInUse).
			SetSecretShare(sparkPrivKey).
			SetPublicShares(map[string]keys.Public{"operator1": sparkPrivKey.Public()}).
			SetPublicKey(sparkPrivKey.Public()).
			SetMinSigners(2).
		...
		leaf, err := sessionCtx.Client.TreeNode.Create().
			...
			SetDirectFromCpfpRefundTx(directRefundTx).
			SetVout(leafVout).
			Save(ctx)
		require.NoError(t, err)
		...
		userPublicKeys := map[string]keys.Public{
			leaf.ID.String(): userPrivKey.Public(),
		}
		tweaked, err := handler.checkIfKeyTweakApplied(ctx, transfer, nil, userPublicKeys)
		require.NoError(t, err)
		assert.True(t, tweaked)
	})

	t.Run("returns false when combined key does not match", func(t *testing.T) {
		sparkPrivKey := keys.MustGeneratePrivateKeyFromRand(rng)
		userPrivKey := keys.MustGeneratePrivateKeyFromRand(rng)
		wrongUserPrivKey := keys.MustGeneratePrivateKeyFromRand(rng)

		// Set verifying pubkey to spark + user, but pass wrongUser.
		combinedPubKey := sparkPrivKey.Public().Add(userPrivKey.Public())
		...
		userPublicKeys := map[string]keys.Public{
			leaf.ID.String(): wrongUserPrivKey.Public(),
		}
		tweaked, err := handler.checkIfKeyTweakApplied(ctx, transfer, nil, userPublicKeys)
		require.NoError(t, err)
		assert.False(t, tweaked)
	})

	t.Run("returns error when user public key missing for leaf", func(t *testing.T) {
		sparkPrivKey := keys.MustGeneratePrivateKeyFromRand(rng)
		...
		// Pass empty map — leaf ID not present.
		_, err = handler.checkIfKeyTweakApplied(ctx, transfer, nil, map[string]keys.Public{})
		require.Error(t, err)
		assert.Contains(t, err.Error(), "user public key for leaf")
	})
}
