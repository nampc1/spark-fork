...
// runRenewWithBothPaths runs testFn twice: once on the legacy path (knob=0),
...
func runRenewWithBothPaths(t *testing.T, testFn func(t *testing.T)) {
	t.Run("legacy", func(t *testing.T) {
		kc, err := sparktesting.NewKnobController(t)
		if err == nil {
			err = kc.SetKnob(t, knobs.KnobUseConsensusRenew, 0)
	...
	t.Run("consensus", func(t *testing.T) {
		...
		defer func() {
			err := kc.SetKnob(t, knobs.KnobUseConsensusRenew, 0)
			assert.NoError(t, err, "failed to reset KnobUseConsensusRenew")
		}()
		testFn(t)
	})
}
...
func TestRenewNodeZeroTimelock(t *testing.T) {
	runRenewWithBothPaths(t, func(t *testing.T) {
